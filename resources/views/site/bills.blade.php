<x-site-second-layout>

    <section class="container mx-auto mt-5 w-90 relative flex justify-center">
        <div class="card__content w-full h-full flex flex-col justify-center items-center" style="margin-top: 100px">
            <div class="card__title">
                <h2 class="text-black text-xl font-bold">تواصل معنا</h2>
            </div>
            <div class="card__form p-4 flex justify-around flex-col items-center w-full">
                <div class="flex flex-wrap justify-center gap-4 w-full">
                    @foreach ($orders as $order)
                    <div class="card bg-[#eeeded3d] w-1/3 rounded-lg shadow-lg py-4 px-2">
                        <div class="card__content">
                            <div class="flex justify-between w-2/4 p-2 rounded-full bg-gray-300">
                                <span class="text-gray-500 font-bold">
                                    {{ $order->created_at->format('h:i A') }}
                                </span>
                                <span class="text-gray-500 font-bold">
                                    {{ $order->created_at->format('d/m/Y') }}
                                </span>
                            </div>
                            <ul class="w-full text-surface dark:text-white">
                                @foreach ($order->orderIteams as $item)
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4 px-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">
                                        {{ \App\Models\Product::find($item->product_id)->name_en }}
                                    </span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">
                                        {{$item->quantity}}
                                    </span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">
                                        {{$item->price}}
                                    </span>
                                </li>
                                @endforeach
                            </ul>
                            <div class="flex justify-between w-full p-4">
                                <span class="font-bold text-black text-lg">المجموع</span>
                                <span class="font-bold text-red-600 text-xl p-3 bg-white">
                                    {{ $order->total_amount }}
                                </span>
                            </div>
                        </div>
                    </div>
                    @endforeach
                    {{-- <div class="card bg-[#eeeded3d] w-1/3 rounded-lg shadow-lg py-4 px-2">
                        <div class="card__content">
                            <div class="flex justify-between w-2/4 p-2 rounded-full bg-gray-300">
                                <span class="text-gray-500 font-bold">2:54 PM</span>
                                <span class="text-gray-500 font-bold">8/8/2024</span>
                            </div>
                            <ul class="w-full text-surface dark:text-white">
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">وجبة الملكي</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">1</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">بيتزا</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">2</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">مشروبات</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">3</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                            </ul>
                            <div class="flex justify-between w-full p-4">
                                <span class="font-bold text-black text-lg">المجموع</span>
                                <span class="font-bold text-red-600 text-xl p-3 bg-white">$157</span>
                            </div>
                        </div>
                    </div> --}}
                    {{-- <div class="card bg-[#eeeded3d] w-1/3 rounded-lg shadow-lg py-4 px-2">
                        <div class="card__content">
                            <div class="flex justify-between w-2/4 p-2 rounded-full bg-gray-300">
                                <span class="text-gray-500 font-bold">2:54 PM</span>
                                <span class="text-gray-500 font-bold">8/8/2024</span>
                            </div>
                            <ul class="w-full text-surface dark:text-white">
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">وجبة الملكي</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">1</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">بيتزا</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">2</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">مشروبات</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">3</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                            </ul>
                            <div class="flex justify-between w-full p-4">
                                <span class="font-bold text-black text-lg">المجموع</span>
                                <span class="font-bold text-red-600 text-xl p-3 bg-white">$157</span>
                            </div>
                        </div>
                    </div> --}}
                    {{-- <div class="card bg-[#eeeded3d] w-1/3 rounded-lg shadow-lg py-4 px-2">
                        <div class="card__content">
                            <div class="flex justify-between w-2/4 p-2 rounded-full bg-gray-300">
                                <span class="text-gray-500 font-bold">2:54 PM</span>
                                <span class="text-gray-500 font-bold">8/8/2024</span>
                            </div>
                            <ul class="w-full text-surface dark:text-white">
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">وجبة الملكي</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">1</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">بيتزا</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">2</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                                <li class="flex justify-between items-center w-full border-b-2 border-neutral-100 py-4  hover:ps-2 transition-all delay-150 ease-in">
                                    <span class="text-black text-base font-normal w-1/3">مشروبات</span>
                                    <span class="text-black text-base font-normal w-1/3" align="center">3</span>
                                    <span class="text-black text-base font-normal w-1/3" align="left">$15</span>
                                </li>
                            </ul>
                            <div class="flex justify-between w-full p-4">
                                <span class="font-bold text-black text-lg">المجموع</span>
                                <span class="font-bold text-red-600 text-xl p-3 bg-white">$157</span>
                            </div>
                        </div>
                    </div> --}}
                </div>
                <div class="flex items-center justify-end w-full p-4">
                    <a href="#" class="text-sacndary font-bold text-lg hover:underline dark:text-white">
                        مسح الكل
                    </a>
                </div>
            </div>
        </div>
    </section>

</x-site-second-layout>
